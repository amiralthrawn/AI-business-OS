"""Proof that the Business State Snapshot is fed by the Interpretation
Engine's Business Events -- a richer "interpretation" area supersedes the
plain "observation" one for the same entity, and both still defer to an
existing open Risk/Opportunity from the older per-metric rules, exactly like
app.observation's own Snapshot integration (see
tests/test_observation_snapshot_integration.py and
brain/interpretation_engine.md)."""
...
def test_snapshot_shows_a_richer_interpretation_area_for_an_uncovered_entity(db_session, session_factory):
    ...
    run_interpretation_sweep(db_session, bus, build_capability_registry(), DeterministicLLMClient(), company.id)

    snapshot = build_snapshot(db_session, company.id)

    interpretation_areas = [a for a in snapshot.areas if a.kind == "interpretation"]
    assert len(interpretation_areas) == 1
    area = interpretation_areas[0]
    assert area.interpretation_type == "risk"
    assert area.explanation
    assert area.recommendation
    assert area.entity_id == product.id
    assert area in snapshot.material_areas

    # Superseded, not duplicated: no plain "observation" area for the same event.
    assert not any(a.kind == "observation" for a in snapshot.areas)


def test_snapshot_still_prefers_an_existing_open_risk_over_the_interpretation(db_session, session_factory):
    bus = _bus(session_factory)
    company, supplier, product, customer = _seed_margin_risk_scenario(db_session)
    run_observation_sweep(db_session, bus, build_observable_registry(), company.id)
    run_interpretation_sweep(db_session, bus, build_capability_registry(), DeterministicLLMClient(), company.id)
